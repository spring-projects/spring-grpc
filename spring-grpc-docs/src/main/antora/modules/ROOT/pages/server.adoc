...
For more specialized configuration, you can provide a javadoc:org.springframework.grpc.server.ServerBuilderCustomizer[] bean to customize the `ServerBuilder` before it is used to create the server.
...
Spring Boot gRPC includes autoconfiguration that configures the server to use the servlet container if it detects that it is in a web application.

To run in this mode you must enable HTTP2 on the server by setting `server.http2.enabled` to `true` and include the required dependencies as follows:
...
A javadoc:org.springframework.grpc.server.InProcessGrpcServerFactory[] picks up the javadoc:org.springframework.grpc.server.service.ServerInterceptorFilter[] automatically.
Any other server factory will require you to provide a `GrpcServerFactoryCustomizer` in which you can modify the factory by adding a filter, as shown in the following example:
...
You can change the defaults and add your own rules by configuring beans of type `UserDetailsService` and/or javadoc:org.springframework.grpc.server.security.AuthenticationProcessInterceptor[].
...
The javadoc:org.springframework.grpc.server.security.AuthenticationProcessInterceptor[] can be created from a Spring Security configurer of type javadoc:org.springframework.grpc.server.security.GrpcSecurity[].
