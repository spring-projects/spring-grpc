...
public class ConfigMapConverterTests {

	private GrpcClientProperties.ConfigMapConverter converter = new GrpcClientProperties.ConfigMapConverter();

	@Test
	void testSimple() {
		Map<String, Object> map = converter.convert(Map.of("userAgent", "foo", "maxInboundMessageSize", "1MB"));
		assertThat(map).containsEntry("userAgent", "foo").containsEntry("maxInboundMessageSize", "1MB");
	}

	@Test
	void testList() {
		Map<String, Object> map = converter.convert(Map.of("spam", Map.of("0", "foo", "1", "bar")));
		assertThat(map.get("spam")).asInstanceOf(InstanceOfAssertFactories.LIST).containsExactly("foo", "bar");
	}

	@Test
	void testNotReallyAList() {
		Map<String, Object> map = converter.convert(Map.of("spam", Map.of("0", "foo", "oops", "bar")));
		assertThat(map.get("spam")).asInstanceOf(InstanceOfAssertFactories.MAP).containsKeys("0", "oops");
	}

	@Test
	void testMethodConfig() {
		Map<String, Object> map = converter.convert(Map.of("methodConfig",
				Map.of("0",
